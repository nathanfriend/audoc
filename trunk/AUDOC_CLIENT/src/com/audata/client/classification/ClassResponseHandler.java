...
public class ClassResponseHandler extends BaseRequestCallback {
	...
	public ClassResponseHandler(ClassBrowser browser, TreeItem parent){
	...
	private void addClasses(JSONArray classes){
		...
		for(int i=0;i<classes.size();i++){
			...
			if(this.parent != null){
				this.browser.addClass(name, uuid, this.parent, hasChildren);
			}else{
				this.browser.addClass(name, uuid, hasChildren);
			}
		}
		this.browser.show();
	}
